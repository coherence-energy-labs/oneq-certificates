...
Code, artifacts, conformance vectors, the seeded-fault corpus and the certificate schema are released under the Apache License 2.0. Every number in this paper is typeset from a macro generated from a JSON artifact, and each artifact records the command and code that produced it; the certification-semantics audit (Table~\ref{tab:semantics}) re-runs the producing code of each headline artifact from its commit. The Willow data are read in place from the published archive~\cite{google2024data}. Validating a certificate against its schema establishes only that it is well formed; establishing that its claim is true requires re-running a checker against a graph the verifier supplies.

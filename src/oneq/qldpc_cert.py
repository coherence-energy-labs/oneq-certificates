...
#: How far from an integer an LP coordinate may sit and still be read as
#: that integer. One constant, one place -- see `_integral_support`.
INTEGRALITY_TOL = 1e-7


def _integral_support(x, var_ids) -> tuple[int, ...] | None:
    """The support of an INTEGRAL LP point, or None if it is fractional.

    *** THIS RULE WAS WRITTEN TWICE. *** `feldman_dual_lazy` and
    `feldman_dual` each carried their own copy, character for character,
    tolerance included. They agreed on the day they were found and
    nothing made them agree tomorrow -- and this estate has already
    watched two copies of "one strict parser" drift apart inside a week.

    The rule itself is load-bearing rather than cosmetic. The Feldman LP's
    optimum can be half-integral: the canonical LP-decoding failure, a
    PSEUDOCODEWORD. Rounding one produces a support that satisfies every
    parity check and costs more than the true optimum -- a plausible
    answer that is not the minimum-weight error. Returning None instead is
    what stops it being handed back as a "repair".

    `var_ids[k]`, never `k`: an internal soundness audit found the repair
    support built from LP COLUMN indices, which is harmless while every
    caller uses contiguous 0..n-1 ids and wrong the day one does not.
    """
    import numpy as np              # per-function, as every caller here does
    if np.max(np.abs(x - np.round(x))) >= INTEGRALITY_TOL:
        return None                 # fractional: a pseudocodeword, no repair
    return tuple(int(var_ids[k]) for k in np.flatnonzero(np.round(x) > 0.5))
...
def feldman_dual_lazy(checks, weights, syndrome, *, max_rounds: int = 60,
    ...
    primal = _integral_support(x, var_ids)
